...
app.mount("/psl_images", StaticFiles(directory="psl_images"), name="psl_images")
...
async def process_frame(frame_data: str, connection_id: str) -> str:
    ...
    try:
        ...
        # CRITICAL FIX: Flip frame horizontally to match laptop front camera training
...
# Load SIBI model once
try:
    sibi_model = load_model("model_SIBI.h5")
    logger.info("✅ SIBI Model (Text-to-PSL) loaded successfully.")
    MODEL_INPUT_SHAPE = sibi_model.input_shape  # e.g., (None, 26) or (None, 1)
    MODEL_OUTPUT_SHAPE = sibi_model.output_shape
    logger.info(f"Model Input Shape: {MODEL_INPUT_SHAPE}, Output Shape: {MODEL_OUTPUT_SHAPE}")
except Exception as e:
    logger.error(f"❌ Failed to load SIBI model: {e}")
    raise

# Folder containing A-Z PSL images
PSL_IMAGES_FOLDER = "psl_images"
PSL_LABELS = [chr(i) for i in range(65, 91)]  # ['A', 'B', ..., 'Z']

@app.post("/text-to-psl")
async def text_to_psl(data: dict = Body(...)):
    """
    Convert text to PSL using the SIBI model and return predicted images.
    """
    ...
    try:
        ...
        for ch in text.upper():
            if ch.isalpha():
                letter_index = PSL_LABELS.index(ch)

                # ✅ Auto-detect model input shape and preprocess accordingly
                if len(MODEL_INPUT_SHAPE) == 2 and MODEL_INPUT_SHAPE[1] == 1:
                    # Numeric encoding (e.g., (None, 1))
                    input_data = np.array([[letter_index]], dtype=np.float32)

                elif len(MODEL_INPUT_SHAPE) == 2 and MODEL_INPUT_SHAPE[1] == 26:
                    # One-hot encoding (e.g., (None, 26))
                    input_data = np.zeros((1, 26), dtype=np.float32)
                    input_data[0, letter_index] = 1.0

                else:
                    logger.warning("Unknown input shape. Falling back to direct mapping.")
                    predicted_label = ch
                    input_data = None

                # ✅ Predict if input_data is valid
                if input_data is not None:
                    prediction = sibi_model.predict(input_data)
                    predicted_idx = np.argmax(prediction)
                    predicted_label = PSL_LABELS[predicted_idx]
                else:
                    predicted_label = ch

                # ✅ Map predicted letter to PSL image (fix indentation here!)
                img_url = None
                for ext in [".png", ".jpg"]:
                ...
                output.append({"letter": "SPACE", "image": None})

        return {"input": text, "translation": output}

    except Exception as e:
        logger.error(f"Text-to-PSL error: {e}")
